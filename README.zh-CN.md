...
Gnalloy Channel 的超时与空闲状态 Handler，基于核心 hashed wheel timer。

该模块提供 Pipeline handler。handler 在 Channel 已存在之后观察、转换、拒绝、延迟、记录或保护消息；除非模块名明确说明，否则不拥有监听 socket 或应用协议。

## 状态

- 导入路径：`gnalloy.org/handler-timeout`
- 仓库：`github.com/gnalloy/handler-timeout`
- 默认分支：`dev`
- 预览安装：`go get gnalloy.org/handler-timeout@dev`
- 许可证：Apache-2.0
...
本仓库负责：Gnalloy Channel 的超时与空闲状态 Handler，基于核心 hashed wheel timer。

# ADR-001：保持 handler-timeout 为独立 Gnalloy 模块
...
`handler-timeout` 作为 `gnalloy.org/handler-timeout` 发布，并拥有该职责：Gnalloy Channel 的超时与空闲状态 Handler，基于核心 hashed wheel timer。

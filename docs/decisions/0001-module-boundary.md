# ADR-001: Keep handler-timeout as an Independent Gnalloy Module
...
`handler-timeout` is published as `gnalloy.org/handler-timeout` and owns this responsibility: Timeout and idle-state handlers for Gnalloy channels, backed by the core hashed wheel timer.

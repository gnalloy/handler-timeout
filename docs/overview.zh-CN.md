...
- 模块路径：`gnalloy.org/handler-timeout`
- GitHub 仓库：`github.com/gnalloy/handler-timeout`
- 默认分支：`dev`
- 许可证：Apache-2.0
...
公共导入路径是 `gnalloy.org/handler-timeout`。首个稳定 tag 发布前，请按依赖策略使用 `@dev` 或明确的 pseudo-version。
